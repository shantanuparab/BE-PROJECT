...
class Edrone():
    ...
    def __init__(self):
        ...
        self.setpoint_euler = [0.0, 0.0, 0.0,0.0]
        ...
        self.Kp = [15.72, 45.84, 4500.0]
        self.Ki = [0.152, 0.184, 0.0]
        self.Kd = [353.7, 602.7, 0.0]
        ...
        self.rollerr_pub = rospy.Publisher('/roll_error', Float32, queue_size=1)
        self.pitcherr_pub = rospy.Publisher('/pitch_error',Float32, queue_size=1)
        self.yawerr_pub = rospy.Publisher('/yaw_error', Float32, queue_size=1)
    ...
    def yaw_set_pid(self, yaw):
        rospy.loginfo(yaw.Kp)
        self.Kp[2] = yaw.Kp *0.9  # This is just for an example. You can change the ratio/fraction value accordingly
    ...
    def pid(self):
        ...
        #   7. Don't run the pid continously. Run the pid only at the a sample time. self.sam4pletime defined above is for this purpose. THIS IS VERY IMPORTANT.
        ...
        self.errSum[0]=(self.errSum[0]+self.error[0])*self.Ki[0]
        self.errSum[1]=(self.error[1]+self.error[1])*self.Ki[1]
        self.errSum[2]=(self.error[2]+self.error[2])*self.Ki[2]
        ...
        self.RPY[0]=self.Kp[0]*self.error[0]+self.errSum[0]+(self.Kd[0]*self.dErr[0])
        self.RPY[1]=self.Kp[1]*self.error[1]+self.errSum[1]+(self.Kd[1]*self.dErr[1])
        self.RPY[2]=self.Kp[2]*self.error[2]+self.errSum[2]+(self.Kd[2]*self.dErr[2])

        # Also convert the range of 1000 to 2000 to 0 to 1024 for throttle here itslef
        self.pwm_cmd.prop1 = (self.setpoint_cmd[3]-1000)*1.024-self.RPY[0]+self.RPY[1]-self.RPY[2]
        self.pwm_cmd.prop4 = (self.setpoint_cmd[3]-1000)*1.024+self.RPY[0]+self.RPY[1]+self.RPY[2]
        self.pwm_cmd.prop2 = (self.setpoint_cmd[3]-1000)*1.024-self.RPY[0]-self.RPY[1]+self.RPY[2]
        self.pwm_cmd.prop3 = (self.setpoint_cmd[3]-1000)*1.024+self.RPY[0]-self.RPY[1]-self.RPY[2]
    	if self.pwm_cmd.prop1>1024.0 :
    		self.pwm_cmd.prop1=1024.0
        if self.pwm_cmd.prop1<0.0 :
    		self.pwm_cmd.prop1=0.0
    	if self.pwm_cmd.prop2>1024.0 :
    		self.pwm_cmd.prop2=1024.0
        if self.pwm_cmd.prop2<0.0 :
    		self.pwm_cmd.prop2=0.0
    	if self.pwm_cmd.prop3>1024.0 :
    		self.pwm_cmd.prop3=1024.0
        if self.pwm_cmd.prop3<0.0 :
    		self.pwm_cmd.prop3=0.0
    	if self.pwm_cmd.prop4>1024.0 :
    		self.pwm_cmd.prop4=1024.0
        if self.pwm_cmd.prop4<0.0 :
		    self.pwm_cmd.prop4=0.0
        # rospy.loginfo(self.pwm_cmd.prop4)
        # rospy.loginfo(self.pwm_cmd.prop1)
        # rospy.loginfo(self.pwm_cmd.prop2)
        # rospy.loginfo(self.pwm_cmd.prop3)
        # rospy.loginfo(self.error)
        # rospy.loginfo("--------------------------------------")
        self.pwm_pub.publish(self.pwm_cmd)
        self.rollerr_pub.publish(self.error[0])
        self.pitcherr_pub.publish(self.error[1])
        self.yawerr_pub.publish(self.error[2])
